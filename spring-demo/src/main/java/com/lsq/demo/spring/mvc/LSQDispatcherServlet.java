...
public class LSQDispatcherServlet {
    ...
    public void doDispath(HttpServletResponse response, HttpServletRequest request) throws Exception {

        LSQHandlerMapping handler = getHndlerMapping(request);

        LSQHandlerAdapter ha = getHandlerAdapter(handler);

        LSQModelAndView mc = ha.handle(response, request, handler);

        processDispathchReuslt(response, mc);
    }

    private void processDispathchReuslt(HttpServletResponse response, LSQModelAndView mc) {
        if (mc == null) {
            return;
        }
        ...
        for (LSQViewResolver viewResolver : viewResolvers) {
            if (!mc.getViewName().equals(viewResolver.getName())) {
                continue;
            }
    ...
    private LSQHandlerMapping getHndlerMapping(HttpServletRequest request) {
    ...
    private LSQHandlerAdapter getHandlerAdapter(LSQHandlerMapping handler) {
        if (CollectionUtils.isEmpty(handlerAdapterMap)) {
            return null;
        }
